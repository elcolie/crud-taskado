...
from test_gadgets import remove_all_tasks_and_users, prepare_users_for_test
...
class TestPost(unittest.TestCase):
    ...
    def test_post_invalid_created_by(self) -> None:
        ...
        assert response.json() == {'detail': [{'type': 'value_error', 'loc': ['body', 'created_by'],
                                               'msg': 'Value error, User with this id does not exist', 'input': 999,
                                               'ctx': {'error': {}}}]}
    ...
    def test_invalid_status(self) -> None:
        ...
        assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY
        assert response.json() == {'detail': [{'type': 'enum', 'loc': ['body', 'status'],
    ...
    def test_invalid_status_and_due_date_and_wrong_created_by(self) -> None:
        ...
        assert response.json() == {'detail': [{'type': 'enum', 'loc': ['body', 'status'],
                                              ...
                                              {'type': 'value_error', 'loc': ['body', 'due_date'],
                                               'msg': "Value error, Invalid date format. Must be in 'YYYY-MM-DD' format.",
                                               'input': '2022-12-31T00:00:00', 'ctx': {'error': {}}},
                                              {'type': 'value_error', 'loc': ['body', 'created_by'],
                                               'msg': 'Value error, User with this id does not exist', 'input': 999,
                                               'ctx': {'error': {}}}]}
